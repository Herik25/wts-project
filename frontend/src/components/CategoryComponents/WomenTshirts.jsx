...
import {
  ...
  fetchAllWomensTshirtsAsync,
  fetchWomenTshirtsByFiltersAsync,
  selectAllProducts,
...
const filters = [
  {
    ...
    name: "Category",
    options: [
      { value: "tshirt", label: "T-Shirt", checked: true },
...
function WomenTshirts() {
  ...
  const [filter, setFilter] = useState({ category: "tshirt" });
  ...
  useEffect(() => {
    dispatch(fetchAllWomensTshirtsAsync());
  }, [dispatch]);

  useEffect(() => {
    dispatch(fetchWomenTshirtsByFiltersAsync(filter));
  }, [filter, dispatch]);
